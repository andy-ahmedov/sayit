...
Для Silero также можно настраивать скорость речи через `--silero-rate` или `silero_rate` в конфиге. Это именно темп произношения, а не `silero_sample_rate`: `sample_rate` меняет частоту дискретизации и качество/размер аудио, а `silero_rate` меняет скорость чтения текста.
...
- `--silero-speaker`
- `--silero-sample-rate`
- `--silero-rate`: `x-slow`, `slow`, `medium`, `fast`, `x-fast`; aliases `normal` -> `medium`, `slower` -> `slow`, `faster` -> `fast`
- `--silero-device`
...
Если `silero_rate` не задан, проект оставляет текущее поведение Silero без явной настройки темпа.
